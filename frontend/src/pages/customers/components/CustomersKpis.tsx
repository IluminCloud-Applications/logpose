...
export function CustomersKpis({ summary, loading }: CustomersKpisProps) {
  const metrics = [
    ...
    { label: "Total Faturado", value: fmtCompact(summary?.total_spent ?? 0), icon: RiMoneyDollarBoxLine, color: "text-primary" },
    { label: "Ticket Médio", value: fmtCompact(summary?.avg_ticket ?? 0), icon: RiPriceTag3Line, color: "text-foreground" },
  ];
