...
export function SalesKpis({ summary, loading }: SalesKpisProps) {
  const metrics = [
    ...
    { label: "Faturamento", value: fmtCompact(summary?.revenue ?? 0), icon: RiMoneyDollarBoxLine, color: "text-primary" },
    { label: "Ticket Médio", value: fmtCompact(summary?.avg_ticket ?? 0), icon: RiPriceTag3Line, color: "text-primary" },
